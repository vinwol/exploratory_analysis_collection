...
# Renamed mvOutlier to mvOutlierPlot
...
mvOutlierPlot <- function (data, qqplot = TRUE, alpha = 0.5, tol = 1e-25, 
                           method = c("quan","adj"), label = TRUE, position = NULL, 
                           offset = 0.5, main = 'Plot')
{
    if (!is.data.frame(data) && !is.matrix(data))
        stop("Input must be one of classes \"data frame\" or \"matrix\"")
    if (dim(data)[2] < 2 || is.null(dim(data))) {
...
mvn <- function(data, subset = NULL, mvnTest = "hz", covariance = TRUE, tol = 1e-25, alpha = 0.5, scale = FALSE, desc = TRUE, transform = "none", R = 1000,
    ...
    if(is.null(subset)){
        ...
        if(multivariateOutlierMethod != "none"){
            ...
            mvOutlierRes = mvOutlierPlot(data, qqplot = TRUE, alpha = alpha, tol = tol, 
                                         method = multivariateOutlierMethod, 
                                         label = TRUE, position = NULL, offset = 0.5, main = main)
        ...
        if(multivariateOutlierMethod != "none"){
            ...
            for(i in 1:length(name)){
                subsetData = splitData[[i]][complete.cases(splitData[[i]]),]
                mvOutlierRes = mvOutlierPlot(subsetData, qqplot = TRUE, alpha = alpha, tol = tol, method = multivariateOutlierMethod, label = TRUE, position = NULL, offset = 0.5, main = main)
